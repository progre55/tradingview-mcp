...
- `quote_get` → single latest price snapshot. Reads ONLY the active chart; a `symbol` arg that doesn't match the active chart errors (fail-closed) rather than relabeling the active chart's data. To quote another symbol, `chart_set_symbol` first, then `quote_get` blank.
...
- `tab_switch` brings a chart tab to the desktop foreground AND repoints the CDP socket, so subsequent `evaluate()` / `ui_evaluate` calls land on the new tab. Without the second step, page-context calls would keep reading the original tab regardless of what's visible to the operator. Switch by `chart_id` (preferred — parsed from the `/chart/<id>/` URL, stable across calls and TV restarts) or `tab_id` (CDP target id, stable within a session); both resolve at call time and the `chart_id` path verifies the attached target before returning. `index` is still accepted but FRAGILE — CDP `/json/list` order is recency/last-click based, so an index from a prior `tab_list` can be stale by the time `tab_switch` runs. This removes the old "re-list, switch, then verify with `chart_get_state`" dance.
- `TV_CHART_ID` (env var, set in the `.mcp.json` `"env"` block — the server reads no shell `.env`) pins CDP target resolution to the `/chart/<id>/` chart regardless of tab focus; unset → the first chart tab (last-focused). Unknown/closed id falls back to the default. Complements `tab_switch`, which repoints the socket at runtime.
