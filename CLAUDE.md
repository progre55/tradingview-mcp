...
- The Pine-editor open helper (`ensurePineEditorOpen`) used by every pine tool auto-recovers from the "Monaco stuck loading" state: after a 3 s initial-mount budget it issues `bwb.hideWidget('pine-editor')` → `bwb.showWidget('pine-editor')` to force a re-mount (the programmatic equivalent of an operator clicking X then reopening the panel — `bwb.showWidget` alone is a no-op on an already-visible widget). It also requires Monaco's `getModel()` to be non-null before declaring the editor ready, since a modelless shell fails `setValue` / `getValue` / `getModelMarkers`. Terminal failures distinguish two states: `pine_editor_monaco_not_ready` (panel mounted but Monaco never finished loading — operator-recoverable by closing and reopening the panel) and "Could not open Pine Editor panel." (bwb never produced the panel). `ui_open_panel({panel: 'pine-editor', wait_for_ready: true})` exposes the same readiness wait to external callers and adds `ready` / `ready_reason` to the response.
